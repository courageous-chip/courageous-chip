...
type Props = { navigation: StackNavigationProp<ParamListBase, "HabitForm"> };

export const HabitFormScreen: FC<Props> = ({ navigation }) => {
  ...
  const onSave: ButtonProps["onPress"] = async () => {
    ...
    try {
      await firebase.firestore().collection("habits").add({ name });
  ...
  return loading ? (
    ...
    <View>
      <TextInput
        ...
        placeholder="🏋️‍♂️ Lift heavy weights"
        value={name}
      />
    </View>
  );
